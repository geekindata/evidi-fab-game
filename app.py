...
def save_user():
    ...
    try:
        ...
        logging.info(f"✅ Received JSON data: {jsonify({k: v for k, v in data.items() if k != 'password'})}")
...
if __name__ == '__main__':
    ...
    logging.info(f"🌐 Server will run on: http://0.0.0.0:{port}")
    logging.info(f"🔍 Debug mode: {'ON' if os.environ.get('FLASK_DEBUG') else 'OFF'}")
    
    if not os.environ.get('SQL_CONNECTION_STRING'):
        logging.warning("⚠️ SQL_CONNECTION_STRING is NOT set - database operations will fail!")
        logging.warning("   Set it with: $env:SQL_CONNECTION_STRING='your-connection-string'")
